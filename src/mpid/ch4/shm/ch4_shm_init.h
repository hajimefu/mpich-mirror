...
static inline int MPIDI_shm_init(int rank,
                                    int size)
{
    ...
    MPIU_CHKPMEM_MALLOC(MPID_nem_mem_region.local_procs, int *, size * sizeof(int), mpi_errno, "local process index array");
    for (i = 0; i < size; ++i) MPID_nem_mem_region.local_procs[i] = i;
    MPID_nem_mem_region.local_rank     = rank;
    MPIU_CHKPMEM_MALLOC (MPID_nem_mem_region.local_ranks, int *, size * sizeof(int), mpi_errno, "mem_region local ranks");
    for (i = 0; i < size; ++i) MPID_nem_mem_region.local_ranks[i] = i;
    MPID_nem_mem_region.next           = NULL;

    /* Request fastboxes region */
    mpi_errno = MPIDI_CH3I_Seg_alloc(MAX((size*((size-1)*sizeof(MPID_nem_fastbox_t))), MPID_NEM_ASYMM_NULL_VAL),
                                     (void **)&fastboxes_p);
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
    
    /* Request data cells region */
    mpi_errno = MPIDI_CH3I_Seg_alloc(size * MPID_NEM_NUM_CELLS * sizeof(MPID_nem_cell_t), (void **)&cells_p);
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
    ...
    MPIU_CHKPMEM_MALLOC(MPID_nem_mem_region.FreeQ, MPID_nem_queue_ptr_t *, size * sizeof(MPID_nem_queue_ptr_t), mpi_errno, "FreeQ");
    MPIU_CHKPMEM_MALLOC(MPID_nem_mem_region.RecvQ, MPID_nem_queue_ptr_t *, size * sizeof(MPID_nem_queue_ptr_t), mpi_errno, "RecvQ");
	...
	MPID_nem_queue_enqueue(MPID_nem_mem_region.FreeQ[rank], &(MPID_nem_mem_region.Elements[idx]));
    }
    ...
    MPIU_CHKPMEM_MALLOC(MPID_nem_mem_region.mailboxes.in,  MPID_nem_fastbox_t **, size * sizeof(MPID_nem_fastbox_t *), mpi_errno, "fastboxes");
    MPIU_CHKPMEM_MALLOC(MPID_nem_mem_region.mailboxes.out, MPID_nem_fastbox_t **, size * sizeof(MPID_nem_fastbox_t *), mpi_errno, "fastboxes");
