...
{
    MPIDI_STATE_DECL(MPID_STATE_MPIDI_POSIX_CELL_INIT);

    MPIDI_FUNC_ENTER(MPID_STATE_MPIDI_POSIX_CELL_INIT);

    MPIDI_POSIX_SET_REL_NULL(cell->next);
    memset((void *) &cell->pkt, 0, sizeof(MPIDI_POSIX_pkt_header_t));
    cell->my_rank = rank;

    MPIDI_FUNC_EXIT(MPID_STATE_MPIDI_POSIX_CELL_INIT);
}
...
{
    MPIDI_STATE_DECL(MPID_STATE_MPIDI_POSIX_QUEUE_INIT);

    MPIDI_FUNC_ENTER(MPID_STATE_MPIDI_POSIX_QUEUE_INIT);

    MPIDI_POSIX_SET_REL_NULL(qhead->head);
    MPIDI_POSIX_SET_REL_NULL(qhead->my_head);
    MPIDI_POSIX_SET_REL_NULL(qhead->tail);

    MPIDI_FUNC_EXIT(MPID_STATE_MPIDI_POSIX_QUEUE_INIT);
}
...
static inline int MPIDI_POSIX_queue_empty(MPIDI_POSIX_queue_ptr_t qhead)
{
    ...
    MPIDI_POSIX_Q_assert(MPIDI_POSIX_IS_REL_NULL(qhead->my_head) || MPIDI_POSIX_IS_REL_NULL(qhead->head));
