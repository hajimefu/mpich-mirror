...
unexpected_l:
    ...
    if(MPIDI_POSIX_mem_region.my_recvQ && !MPIDI_POSIX_queue_empty(MPIDI_POSIX_mem_region.my_recvQ)) {
...
match_l: {
        ...
        char *send_buffer = in_cell ? (char *) cell->pkt.mpich.p.payload : (char *) MPIDI_POSIX_REQUEST(sreq)->user_buf;
        ...
        while(req) {
            ...
            MPL_DBG_MSG_FMT(MPIR_DBG_HANDLE, TYPICAL,
                            (MPL_DBG_FDEST, "Posted from grank %d to %d in progress %d,%d,%d\n",
                             MPIDI_CH4U_rank_to_lpid(sender_rank, req->comm), MPIDI_POSIX_mem_region.rank,
                             sender_rank, tag, context_id));

            if((in_cell && MPIDI_POSIX_ENVELOPE_MATCH(cell, sender_rank, tag, context_id)) ||
               (sreq && MPIDI_POSIX_ENVELOPE_MATCH(MPIDI_POSIX_REQUEST(sreq), sender_rank, tag, context_id))) {
                ...
                if(pending) {
                    ...
                    MPIDI_POSIX_ENVELOPE_SET(MPIDI_POSIX_REQUEST(req_ack), req->comm->rank, tag, context_id);
                ...
                if(MPIDI_POSIX_REQUEST(req)->segment_ptr) {
                    ...
                    MPID_Segment_unpack(MPIDI_POSIX_REQUEST(req)->segment_ptr, MPIDI_POSIX_REQUEST(req)->segment_first, (MPI_Aint *)&last, send_buffer);
                ...
                if(type == MPIDI_POSIX_TYPEEAGER) {
                    ...
                    MPIDI_POSIX_REQUEST_DEQUEUE_AND_SET_ERROR(&req, prev_req, MPIDI_POSIX_recvq_posted,
        ...
        if(in_cell) {
            ...
            MPIDI_POSIX_ENVELOPE_SET(MPIDI_POSIX_REQUEST(rreq), cell->rank, cell->tag, cell->context_id);
            ...
            if(data_sz > 0) {
                ...
                MPIU_Memcpy(MPIDI_POSIX_REQUEST(rreq)->user_buf, (void *) cell->pkt.mpich.p.payload, data_sz);
            } else {
    ...
    if(in_cell) {
        ...
        MPL_DBG_MSG_FMT(MPIR_DBG_HANDLE, TYPICAL,
                        (MPL_DBG_FDEST, "Received from grank %d to %d in progress %d,%d,%d\n", cell->my_rank,
                         MPIDI_POSIX_mem_region.rank, cell->rank, cell->tag, cell->context_id));
        cell->pending = NULL;
        {
        ...
        MPIDI_POSIX_REQUEST_DEQUEUE_AND_SET_ERROR(&sreq, prev_sreq, MPIDI_POSIX_recvq_unexpected, mpi_errno);
    }
...
static inline int MPIDI_POSIX_progress_send(int blocking, int *completion_count)
{
    ...
    if(!MPIDI_POSIX_queue_empty(MPIDI_POSIX_mem_region.my_freeQ)) {
        ...
        MPIDI_POSIX_ENVELOPE_GET(MPIDI_POSIX_REQUEST(sreq), cell->rank, cell->tag, cell->context_id);
        ...
        if(data_sz <= MPIDI_POSIX_EAGER_THRESHOLD) {
            ...
            if(MPIDI_POSIX_REQUEST(sreq)->type == MPIDI_POSIX_TYPEACK) {
                ...
                if(MPIDI_POSIX_REQUEST(sreq)->segment_ptr) {
                    /* non-contig */
                    MPID_Segment_pack(MPIDI_POSIX_REQUEST(sreq)->segment_ptr, MPIDI_POSIX_REQUEST(sreq)->segment_first, (MPI_Aint *)&MPIDI_POSIX_REQUEST(sreq)->segment_size, recv_buffer);
                    MPID_Segment_free(MPIDI_POSIX_REQUEST(sreq)->segment_ptr);
                } else {
                    /* contig */
                    MPIU_Memcpy((void *) recv_buffer, MPIDI_POSIX_REQUEST(sreq)->user_buf, data_sz);
                }
            ...
            /* dequeue sreq */
            MPIDI_POSIX_REQUEST_DEQUEUE_AND_SET_ERROR(&sreq, prev_sreq, MPIDI_POSIX_sendq, mpi_errno);
        } else {
            /* long message */
            if(MPIDI_POSIX_REQUEST(sreq)->segment_ptr) {
                /* non-contig */
                size_t last = MPIDI_POSIX_REQUEST(sreq)->segment_first + MPIDI_POSIX_EAGER_THRESHOLD;
                MPID_Segment_pack(MPIDI_POSIX_REQUEST(sreq)->segment_ptr, MPIDI_POSIX_REQUEST(sreq)->segment_first, (MPI_Aint *)&last, recv_buffer);
                MPIDI_POSIX_REQUEST(sreq)->segment_first = last;
            } else {
                /* contig */
                MPIU_Memcpy((void *) recv_buffer, MPIDI_POSIX_REQUEST(sreq)->user_buf, MPIDI_POSIX_EAGER_THRESHOLD);
