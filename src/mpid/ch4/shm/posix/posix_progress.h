...
match_l: {
            ...
            if((in_cell && MPIDI_POSIX_ENVELOPE_MATCH(cell, sender_rank, tag, context_id)) ||
               (sreq && MPIDI_POSIX_ENVELOPE_MATCH(MPIDI_POSIX_REQUEST(sreq), sender_rank, tag, context_id))) {
                ...
                if(MPIDI_POSIX_REQUEST(req)->segment_ptr) {
                    ...
                    /* contig */
                    if(send_buffer) MPIR_Memcpy(recv_buffer, (void *) send_buffer, data_sz);
        ...
        if(in_cell) {
            ...
            if(data_sz > 0) {
                ...
                MPIR_Memcpy(MPIDI_POSIX_REQUEST(rreq)->user_buf, (void *) cell->pkt.mpich.p.payload, data_sz);
            } else {
    ...
    if(!MPIDI_POSIX_queue_empty(MPIDI_POSIX_mem_region.my_freeQ)) {
        ...
        if(data_sz <= MPIDI_POSIX_EAGER_THRESHOLD) {
            ...
            if(MPIDI_POSIX_REQUEST(sreq)->segment_ptr) {
                ...
                /* contig */
                MPIR_Memcpy((void *) recv_buffer, MPIDI_POSIX_REQUEST(sreq)->user_buf, MPIDI_POSIX_EAGER_THRESHOLD);
