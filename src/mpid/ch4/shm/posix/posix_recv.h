                                                 ...
                                                 MPIR_Comm *comm, int context_offset, MPIR_Request **request)
{
    ...
    MPIDI_POSIX_ENVELOPE_SET(MPIDI_POSIX_REQUEST(rreq), rank, tag, comm->context_id + context_offset);
    rreq->comm = comm;
    ...
    if(!dt_contig) {
        ...
        MPIR_ERR_CHKANDJUMP1((MPIDI_POSIX_REQUEST(rreq)->segment_ptr == NULL), mpi_errno, MPI_ERR_OTHER, "**nomem", "**nomem %s", "MPID_Segment_alloc");
        MPID_Segment_init((char *)buf, MPIDI_POSIX_REQUEST(rreq)->user_count, MPIDI_POSIX_REQUEST(rreq)->datatype, MPIDI_POSIX_REQUEST(rreq)->segment_ptr, 0);
                                       ...
                                       MPIR_Request *message, MPIR_Request **rreqp)
{
    ...
    if(MPIDI_POSIX_REQUEST(message)->pending) {
        ...
        MPIR_Comm_add_ref(message->comm);
        MPIDI_POSIX_ENVELOPE_SET(MPIDI_POSIX_REQUEST(req_ack), message->comm->rank, tag, context_id);
    ...
    for(sreq = message; sreq;) {
        ...
        if(MPIDI_POSIX_REQUEST(sreq)->type == MPIDI_POSIX_TYPEEAGER) {
            ...
            if(MPIDI_POSIX_REQUEST(rreq)->segment_ptr) {
                ...
                MPID_Segment_unpack(MPIDI_POSIX_REQUEST(rreq)->segment_ptr, MPIDI_POSIX_REQUEST(rreq)->segment_first, (MPI_Aint *)&last, send_buffer);
            ...
            if(MPIDI_POSIX_REQUEST(rreq)->segment_ptr) {
                /* non-contig */
                size_t last = MPIDI_POSIX_REQUEST(rreq)->segment_first + MPIDI_POSIX_EAGER_THRESHOLD;
                MPID_Segment_unpack(MPIDI_POSIX_REQUEST(rreq)->segment_ptr, MPIDI_POSIX_REQUEST(rreq)->segment_first, (MPI_Aint *)&last, send_buffer);
                ...
                /* contig */
                if(send_buffer)
                    MPIU_Memcpy(recv_buffer, (void *) send_buffer, MPIDI_POSIX_EAGER_THRESHOLD);
