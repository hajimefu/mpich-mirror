...
unexpected_l:
    if( sreq != NULL ) {
        MPIU_DBG_MSG_FMT(HANDLE,TYPICAL,(MPIU_DBG_FDEST,"Found unexpected in progress req %d,%d,%d\n",REQ_SHM(sreq)->rank,REQ_SHM(sreq)->tag,REQ_SHM(sreq)->context_id));
        goto match_l;
    }
    ...
    for( rank = 0; rank < MPID_nem_mem_region.num_local; rank++ ) {
        if( rank != MPID_nem_mem_region.rank ) { 
            ...
            {
                cell = &fbox->cell;
                MPIU_DBG_MSG_FMT(HANDLE,TYPICAL,(MPIU_DBG_FDEST,"Found fastbox in progress %d,%d,%d\n",cell->rank,cell->tag,cell->context_id));
                in_cell = 1;
                in_fbox = 1;
                goto match_l;
            }
    ...
    if (MPID_nem_mem_region.my_recvQ && !MPID_nem_queue_empty (MPID_nem_mem_region.my_recvQ))
    {
        ...
        MPIU_DBG_MSG_FMT(HANDLE,TYPICAL,(MPIU_DBG_FDEST,"Found cell in progress %d,%d,%d\n",cell->rank,cell->tag,cell->context_id));
        in_cell = 1;
        goto match_l;
    }
    goto fn_exit;
match_l:
    {
        ...
        while( req ) {
            ...
            MPIU_DBG_MSG_FMT(HANDLE,TYPICAL,(MPIU_DBG_FDEST,"Matching in progress req %d,%d,%d\n",sender_rank,tag,context_id));
            if( (in_cell && ENVELOPE_MATCH(cell,sender_rank,tag,context_id)) || (sreq && ENVELOPE_MATCH(REQ_SHM(sreq),sender_rank,tag,context_id)) )
            {
                MPIU_DBG_MSG_FMT(HANDLE,TYPICAL,(MPIU_DBG_FDEST,"Matching in progress done %d,%d,%d\n",sender_rank,tag,context_id));
                ...
                if( type == TYPE_EAGER ) {
                    ...
                    MPIR_STATUS_SET_COUNT(req->status, MPIR_STATUS_GET_COUNT(req->status) + data_sz);
                    /* dequeue rreq */
                    REQ_SHM_DEQUEUE_AND_SET_ERROR(&req,prev_req,MPIDI_shm_recvq_posted,mpi_errno);
                    ...
                    MPIR_STATUS_SET_COUNT(req->status, MPIR_STATUS_GET_COUNT(req->status) + EAGER_THRESHOLD);
                } else {
                    MPIU_Assert(0);
                }
                goto release_cell_l;
        ...
        if( in_cell )
        {
            ...
            MPID_Request* rreq;
            MPIU_DBG_MSG_FMT(HANDLE,TYPICAL,(MPIU_DBG_FDEST,"Unexpected %d,%d,%d\n",cell->rank,cell->tag,cell->context_id));
            ...
            MPIU_Memcpy(REQ_SHM(rreq)->user_buf, (void*)cell->pkt.mpich.p.payload, cell->pkt.mpich.datalen);
...
static inline int MPIDI_shm_do_progress_send ( int blocking, int* completion_count ) {
    ...
    if (!MPID_nem_queue_empty (MPID_nem_mem_region.my_freeQ))
    {
        ...
        MPIU_DBG_MSG_FMT(HANDLE,TYPICAL,(MPIU_DBG_FDEST,"Sending %d,%d,%d\n",cell->rank,cell->tag,cell->context_id));
