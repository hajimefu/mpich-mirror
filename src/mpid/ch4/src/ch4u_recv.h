...
#undef FUNCNAME
#define FUNCNAME MPIDI_CH4U_Prepare_recv_req
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline  int MPIDI_CH4U_Prepare_recv_req(
    void *buf, int count, MPI_Datatype datatype, MPID_Request *rreq)
{
...
#undef FUNCNAME
#define FUNCNAME MPIDI_CH4U_Handle_unexpected
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline  int MPIDI_CH4U_Handle_unexpected(void *buf,
						int count,
...
#undef FUNCNAME
#define FUNCNAME MPIDI_CH4U_Do_irecv
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline  int MPIDI_CH4U_Do_irecv(void *buf,
				       int count,
				       MPI_Datatype datatype,
				       int rank,
				       int tag,
				       MPID_Comm *comm,
				       int context_offset,
				       MPID_Request **request, enum ch4_recv_mode mode,
				       uint64_t flags)
{
    ...
    match_bits = MPID_CH4U_init_recvtag(&mask_bits, context_id, rank, tag);
    comm_idx = MPIDI_CH4U_Get_context_index(comm->recvcontext_id);
    root_comm = MPIDI_CH4_Global.comms[comm_idx];
    
    /* MPIDI_CS_ENTER() */
    rreq = MPIDI_CH4U_Dequeue_unexp_strict(match_bits, mask_bits,
				    &MPIU_CH4U_COMM(root_comm, unexp_list));
    /* MPIDI_CS_EXIT() */

    if (rreq) {
	*request = rreq;
	MPIU_RC_POP(MPIDI_CH4U_Handle_unexpected(buf, count, datatype,
    ...
    *request = rreq;
    ...
    MPIU_CH4U_REQUEST(rreq, datatype) = datatype;
    MPIU_RC_POP( MPIDI_CH4U_Prepare_recv_req(buf, count, datatype, rreq));

    /* MPIDI_CS_ENTER(); */
    MPIDI_CH4U_Enqueue_posted(rreq, &MPIU_CH4U_COMM(root_comm, posted_list));
    /* MPIDI_CS_EXIT(); */
fn_exit:
                                        ...
                                        int context_offset, MPID_Request ** request)
{
    ...
    *request = rreq;
	...
	MPID_CH4U_init_send_tag(comm->context_id + context_offset, rank, tag);
				     ...
				     MPID_Request **rreqp)
{
    ...
    *rreqp = message;
    ...
    if (MPIU_CH4U_REQUEST(message, status) & MPIDI_CH4U_REQ_BUSY) {
        ...
        MPIU_RC_POP(MPIDI_CH4U_Unexp_mrecv_cmpl_handler(message));
    }
    /* MPIDI_CS_EXIT(); */
                                    ...
                                    MPID_Request * message, MPI_Status * status)
{
    ...
    MPIU_RC_POP(MPIDI_Imrecv(buf, count, datatype, message, &rreq));
    ...
    MPIU_RC_POP(MPIR_Request_complete(&req_handle, rreq, status, &active_flag));
...
__CH4_INLINE__ int MPIDI_CH4U_Cancel_recv(MPID_Request *rreq)
{
    ...
    root_comm = MPIDI_CH4_Global.comms[comm_idx];
    found = MPIDI_CH4U_Delete_posted(&rreq->dev.ch4u,
