                                      ...
                                      uint64_t       flags)
{
    ...
    match_bits = MPIDI_CH4U_init_recvtag(&mask_bits, context_id, tag);
    root_comm  = MPIDI_CH4U_context_id_to_comm(comm->recvcontext_id);
    unexp_req  = MPIDI_CH4U_dequeue_unexp(match_bits, mask_bits, rank,
                                        ...
                                        int context_offset, MPID_Request ** request)
{
    ...
    *request = rreq;
    ...
    MPIDI_CH4U_REQUEST(rreq, tag) =
        MPIDI_CH4U_init_send_tag(comm->context_id + context_offset, tag);
    MPIDI_CH4U_REQUEST(rreq, src_rank) = rank;
...
__CH4_INLINE__ int MPIDI_CH4U_cancel_recv(MPID_Request * rreq)
{
    ...
    /* MPIDI_CS_ENTER(); */
    found = MPIDI_CH4U_delete_posted(&rreq->dev.ch4.ch4u.req->rreq, &MPIDI_CH4U_COMM(root_comm, posted_list));
    /* MPIDI_CS_EXIT(); */
