                              ...
                              MPIR_Comm * comm, int context_offset, MPIR_Request ** request)
{
    ...
    if ((r = MPIDI_CH4_rank_is_local(rank, comm)))
        ...
        mpi_errno =
            MPIDI_NM_send(buf, count, datatype, rank, tag, comm, context_offset, request);
                               ...
                               MPIR_Comm * comm, int context_offset, MPIR_Request ** request)
{
    ...
    if ((r = MPIDI_CH4_rank_is_local(rank, comm)))
        ...
        mpi_errno =
            MPIDI_NM_isend(buf, count, datatype, rank, tag, comm, context_offset, request);
                               ...
                               MPIR_Comm * comm, int context_offset, MPIR_Request ** request)
{
    ...
    if ((r = MPIDI_CH4_rank_is_local(rank, comm)))
        ...
        mpi_errno =
            MPIDI_NM_ssend(buf, count, datatype, rank, tag, comm, context_offset, request);
    if(mpi_errno == MPI_SUCCESS && *request) MPIDI_CH4I_REQUEST(*request, is_local) = r;
#endif
    if (mpi_errno != MPI_SUCCESS) {
        MPIR_ERR_POP(mpi_errno);
                                ...
                                MPIR_Comm * comm, int context_offset, MPIR_Request ** request)
{
    ...
    if ((r = MPIDI_CH4_rank_is_local(rank, comm)))
        ...
        mpi_errno =
            MPIDI_NM_issend(buf, count, datatype, rank, tag, comm, context_offset, request);
    if(mpi_errno == MPI_SUCCESS) MPIDI_CH4I_REQUEST(*request, is_local) = r;
#endif
    if (mpi_errno != MPI_SUCCESS) {
        MPIR_ERR_POP(mpi_errno);
...
__CH4_INLINE__ int MPIDI_Startall(int count, MPIR_Request * requests[])
{
    ...
    for(i=0; i<count; i++)
    {
        ...
        if (MPIDI_CH4I_REQUEST_ANYSOURCE_PARTNER(requests[i])) {
            ...
            if (mpi_errno == MPI_SUCCESS) {
                mpi_errno = MPIDI_NM_startall(1, &MPIDI_CH4I_REQUEST_ANYSOURCE_PARTNER(requests[i]));
                MPIDI_CH4I_REQUEST_ANYSOURCE_PARTNER(requests[i]->u.persist.real_request) =
                    MPIDI_CH4I_REQUEST_ANYSOURCE_PARTNER(requests[i])->u.persist.real_request;
                MPIDI_CH4I_REQUEST_ANYSOURCE_PARTNER(MPIDI_CH4I_REQUEST_ANYSOURCE_PARTNER(requests[i])->u.persist.real_request) =
