                                    ...
                                    MPID_Win * win, MPID_Request ** request)
{
    ...
    offset = target_disp * MPIDI_CH4U_WINFO_DISP_UNIT(win, target_rank);
    ...
    if (HANDLE_GET_KIND(target_datatype) == HANDLE_KIND_BUILTIN) {
        ...
        mpi_errno = MPIDI_CH4_NM_send_am(target_rank, win->comm_ptr, MPIDI_CH4U_PUT_REQ,
    ...
    MPIDU_Segment_init((void *)am_hdr.addr, target_count, target_datatype, segment_ptr, 0);
    last = data_sz;
    ...
    if ((am_iov[0].iov_len + am_iov[1].iov_len) <= MPIDI_CH4_NM_am_hdr_max_sz()) {
        mpi_errno = MPIDI_CH4_NM_send_amv(target_rank, win->comm_ptr, MPIDI_CH4U_PUT_REQ,
                         ...
                         MPID_Request **request)
{
    ...
    mpi_errno = MPIDI_CH4_NM_send_am(target_rank, win->comm_ptr, MPIDI_CH4U_GET_REQ,
                                            ...
                                            MPID_Request *sreq)
{
    ...
    if ((data_sz == 0 && do_get == 0) ||
        ...
        (do_get == 1 && origin_count == 0 && MPIDI_CH4U_REQUEST(sreq, req->areq.result_count) == 0)) {
        if (do_get)
    ...
    if (do_get) {
        MPIDI_Datatype_check_size(MPIDI_CH4U_REQUEST(sreq, req->areq.result_datatype),
                                  MPIDI_CH4U_REQUEST(sreq, req->areq.result_count),
                                  result_data_sz);
    ...
    if (HANDLE_GET_KIND(target_datatype) == HANDLE_KIND_BUILTIN) {
        ...
        mpi_errno = MPIDI_CH4_NM_send_am(target_rank, win->comm_ptr, MPIDI_CH4U_ACC_REQ,
                                         &am_hdr, sizeof(am_hdr), origin_addr,
    ...
    MPIDU_Segment_init((void *)am_hdr.target_addr, target_count, target_datatype, segment_ptr, 0);
    last = data_sz;
    MPIDU_Segment_count_contig_blocks(segment_ptr, 0, &last, &num_iov);
    n_iov = (int) num_iov;
    MPIU_Assert(n_iov > 0);
    ...
    if ((am_iov[0].iov_len + am_iov[1].iov_len) <= MPIDI_CH4_NM_am_hdr_max_sz()) {
        mpi_errno = MPIDI_CH4_NM_send_amv(target_rank, win->comm_ptr, MPIDI_CH4U_ACC_REQ,
                                               ...
                                               MPI_Aint target_disp, MPID_Win * win)
{
    ...
    mpi_errno = MPIDI_CH4_NM_send_am(target_rank, win->comm_ptr, MPIDI_CH4U_CSWAP_REQ,
