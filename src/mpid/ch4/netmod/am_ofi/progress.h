...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int MPIDI_netmod_handle_ack_msg(MPIDI_AM_OFI_hdr_t * msg_hdr, fi_addr_t source)
{
    int mpi_errno = MPI_SUCCESS;
    MPID_Request *sreq;
    ...
    if (handler_id == MPIDI_AMTYPE_LMT_ACK && AMREQ_OFI(sreq, pack_buffer)) {
...
static inline int MPIDI_netmod_dispatch_ack(fi_addr_t source,
                                            MPIDI_OFIAMReq_t * ofi_req, void *netmod_context)
{
    int mpi_errno = MPI_SUCCESS;
...
static inline int MPIDI_netmod_progress(void *netmod_context, int blocking)
{
    ...
    do {
        ...
        if (cq_entry.flags & FI_SEND) {
            ...
            MPI_RC_POP(MPIDI_netmod_handle_recv_completion(&cq_entry, source, netmod_context));

            if (cq_entry.flags & FI_MULTI_RECV) {
                MPI_RC_POP(MPIDI_netmod_repost_buffer(cq_entry.op_context, netmod_context));
            }

        }
        else if (cq_entry.flags & FI_READ) {
            MPI_RC_POP(MPIDI_netmod_handle_read_completion(&cq_entry, source, netmod_context));
