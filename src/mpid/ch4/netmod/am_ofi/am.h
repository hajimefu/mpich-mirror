...
static inline int MPIDI_netmod_reg_hdr_handler(
    int handler_id,
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static inline int MPIDI_netmod_ofi_send_am_hdr(
    int64_t rank, int handler_id, const void *am_hdr, size_t am_hdr_sz,
    MPID_Request *sreq)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    FI_RC_RETRY(fi_sendv(MPIDI_Global.ep, iov, NULL, 2,
			 rank, &AMREQ_OFI(sreq, context)), sendv);
fn_exit:
						...
						MPID_Request *sreq)
{
    ...
    FI_RC_RETRY(fi_sendv(MPIDI_Global.ep, iov, NULL, 3,
			 rank, &AMREQ_OFI(sreq, context)), sendv);
fn_exit:
					   ...
					   MPID_Request *sreq)
{
	...
	MPIDI_netmod_ofi_send_am_short(rank, handler_id,
				       ...
				       sreq) :
	MPIDI_netmod_ofi_send_am_long(rank, handler_id,
				      am_hdr, am_hdr_sz, send_buf, data_sz,
				      sreq);
fn_exit:
                                           ...
                                           MPID_Request *sreq, void *netmod_context)
{
    ...
    mpi_errno = MPIDI_netmod_ofi_send_am_hdr(rank, handler_id,
