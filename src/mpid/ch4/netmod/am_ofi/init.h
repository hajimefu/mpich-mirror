...
static inline int MPIDI_Choose_provider(struct fi_info *prov,
				    ...
				    int num_contexts, void **netmod_contexts)
{
    ...
    CH4_COMPILE_TIME_ASSERT(sizeof(MPID_Request) >=
			    (offsetof(MPID_Request,dev.ch4u.netmod_am) +
			     sizeof(MPIDI_OFIAMReq_t)));
	...
	(char*)MPIU_Strdup(MPIR_CVAR_OFI_USE_PROVIDER) :
	NULL;
    ...
    MPI_RC_POP(MPIDI_Create_endpoint(prov_use, MPIDI_Global.domain, MPIDI_Global.am_cq,
				     MPIDI_Global.mr, MPIDI_Global.av,
				     &MPIDI_Global.ep));
	...
	PMI_RC(PMI_KVS_Get(MPIDI_Global.kvsname, keyS, valS,
			   MPIDI_KVSAPPSTRLEN), pmi);
					...
					struct fid_av *av, struct fid_ep **ep)
{
    ...
    FI_RC(fi_ep_bind(*ep, &cq->fid, FI_SEND | FI_RECV |
		     FI_WRITE | FI_READ), cq_bind);
