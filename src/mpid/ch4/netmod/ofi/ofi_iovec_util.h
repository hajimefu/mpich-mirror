                                     ...
                                     size_t                          *target_iovs_nout)
{
    ...
    if(rc == MPIDI_OFI_IOV_DONE) return MPIDI_OFI_IOV_DONE;
    else {
        iov_state->buf_limit_left   = iov_state->buf_limit;
        return MPIDI_OFI_IOV_EAGAIN;
    }
                                      ...
                                      size_t                          *target_iovs_nout)
{
    ...
    rc = MPIDI_OFI_next_iovec_state2(iov_state,&origin_last_addr,&result_last_addr,&target_last_addr, &last_len);
    assert(rc!=MPIDI_OFI_IOV_ERROR);
    MPIDI_OFI_INIT_IOV(target);
    MPIDI_OFI_INIT_IOV(origin);
