                                                 ...
                                                 int         do_tagged)
{
    ...
    *tag_ub = (1 << MPIDI_OFI_TAG_SHIFT) - 1;
    ...
    /* ------------------------------------------------------------------------ */
    MPIDI_OFI_CALL(fi_domain(MPIDI_Global.fabric, prov_use, &MPIDI_Global.domain, NULL), opendomain);
    ...
    for(i = 0; i < size; i++) {
        ...
        MPIDI_OFI_PMI_CALL_POP(PMI_KVS_Get(MPIDI_Global.kvsname, keyS, valS, MPIDI_KVSAPPSTRLEN), pmi);
                                                    ...
                                                    int              do_scalable_ep)
{
    ...
    if(do_scalable_ep) {
        ...
        tx_attr.caps     |= FI_DELIVERY_COMPLETE;
        tx_attr.op_flags  = FI_DELIVERY_COMPLETE;
        MPIDI_OFI_CALL(fi_tx_context(*ep, index + 1, &tx_attr, &MPIDI_OFI_EP_TX_RMA(index), NULL), ep);
        MPIDI_OFI_CALL(fi_ep_bind(MPIDI_OFI_EP_TX_RMA(index), &p2p_cq->fid, FI_SEND), bind);

        tx_attr          = *prov_use->tx_attr;
        tx_attr.caps     = FI_MSG;
        tx_attr.op_flags = 0;
        MPIDI_OFI_CALL(fi_tx_context(*ep, index + 2, &tx_attr, &MPIDI_OFI_EP_TX_MSG(index), NULL), ep);
        ...
        tx_attr.caps     |= FI_DELIVERY_COMPLETE;
        tx_attr.op_flags  = FI_DELIVERY_COMPLETE;
        MPIDI_OFI_CALL(fi_tx_context(*ep, index + 3, &tx_attr, &MPIDI_OFI_EP_TX_CTR(index), NULL), ep);
        ...
        rx_attr.op_flags  = 0;
        MPIDI_OFI_CALL(fi_rx_context(*ep, index + 1, &rx_attr, &MPIDI_OFI_EP_RX_RMA(index), NULL), ep);
        ...
        MPIDI_OFI_CALL(fi_rx_context(*ep, index + 2, &rx_attr, &MPIDI_OFI_EP_RX_MSG(index), NULL), ep);
        ...
        rx_attr.caps     |= FI_ATOMICS;
        rx_attr.op_flags  = 0;
        MPIDI_OFI_CALL(fi_rx_context(*ep, index + 3, &rx_attr, &MPIDI_OFI_EP_RX_CTR(index), NULL), ep);
        ...
        MPIDI_OFI_CALL(fi_ep_bind(*ep, &p2p_cq->fid,  FI_SEND | FI_RECV | FI_SELECTIVE_COMPLETION), bind);
