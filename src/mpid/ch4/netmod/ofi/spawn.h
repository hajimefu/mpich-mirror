...
static inline int MPIDI_OFI_get_tag_from_port(const char *port_name,
                                                      int        *port_name_tag)
{
    int mpi_errno = MPI_SUCCESS;
...
static inline int MPIDI_OFI_get_conn_name_from_port(const char *port_name,
                                                            char       *connname)
{
    int mpi_errno = MPI_SUCCESS;
                                                  ...
                                                  MPID_Node_id_t **out_node_table)
{
    ...
    MPIDI_OFI_MPI_CALL_POP(MPIR_Bcast_intra(out_root,1,MPI_INT,
                                            ...
                                            MPIR_Comm **newcomm)
{
    ...
    if(rank == root) {
        ...
        MPIDI_OFI_MPI_CALL_POP(MPIDI_OFI_dynproc_exchange_map(root,1,port_id,&conn,conname,comm_ptr,
                                                                              ...
                                                                              &parent_addr_table,
                                                                              &parent_node_table));
        MPIDI_OFI_MPI_CALL_POP(MPIDI_OFI_dynproc_exchange_map(root,0,port_id,&conn,conname,comm_ptr,
                                                                              &parent_table_sz,&parent_root,
                                                                              &parent_addr_table,
                                                                              &parent_node_table));
