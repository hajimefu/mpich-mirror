                                              ...
                                              uint64_t peek_flags)
{
    ...
    match_bits = MPIDI_OFI_init_recvtag(&mask_bits, comm->context_id + context_offset, source, tag, MPIDI_OFI_ENABLE_DATA);
    ...
    MPIDI_OFI_CALL(fi_trecvmsg(MPIDI_OFI_EP_RX_TAG(0), &msg, peek_flags | FI_PEEK | FI_COMPLETION | MPIDI_OFI_ENABLE_DATA), trecvmsg);
    ...
    switch(MPIDI_OFI_REQUEST(rreq, util_id)) {
        case  MPIDI_OFI_PEEK_NOT_FOUND:
            *flag = 0;
            ...
            if(message) MPIR_Handle_obj_free(&MPIR_Request_mem, rreq);
